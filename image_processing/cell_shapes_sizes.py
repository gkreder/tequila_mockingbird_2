...
def get_eccentricities_sizes(labeled_img):
    eccentricities = []
    sizes = []
    regionprops = skimage.measure.regionprops(labeled_img)
    for region in regionprops:
        # masked_image = get_masked_cell_image(intensity_img_masked_norm, labeled_cells, region.label)
        # cell_intensity_sum = np.sum(masked_image)
        cell_eccentricity = region.eccentricity
        eccentricities.append(cell_eccentricity)
        sizes.append(region.area)
    return eccentricities, sizes
...
for filename in nd2_files:
    ...
    if row not in ['A', 'B']:
        for i in range(0,6,1):
            ...
            eccentricities, sizes = get_eccentricities_sizes(j)
            print row
            if row in ['C', 'D']:
                DMSO_sizes.append(sizes)
                DMSO_eccentricities.append(eccentricities)
                print 'added to DMSO'
            if row in ['E', 'F', 'G', 'H']:
                PERTURB_sizes.append(sizes)
                PERTURB_eccentricities.append(eccentricities)
                print 'added to PERTURB'

# gfp_intensities = np.concatenate(gfp_intensities)
DMSO_sizes = np.concatenate(DMSO_sizes)
DMSO_eccentricities = np.concatenate(DMSO_eccentricities)
PERTURB_sizes = np.concatenate(PERTURB_sizes)
PERTURB_eccentricities = np.concatenate(PERTURB_eccentricities)
# gfp_intensities = np.concatenate(gfp_intensities)
...
pickle.dump(DMSO_sizes, open('DMSO_sizes.pkl', 'wb'))
pickle.dump(DMSO_eccentricities, open('DMSO_eccentricities.pkl', 'wb'))
pickle.dump(PERTURB_sizes, open('PERTURB_sizes.pkl', 'wb'))
pickle.dump(PERTURB_eccentricities, open('PERTURB_eccentricities.pkl', 'wb'))
